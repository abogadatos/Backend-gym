...
export class ReviewsService {
  ...
  async initializeReviews() {
    for (const reviewData of reviewsData) {
      ...
      const class_entity = await this.classesRepository.findOne({
        where: { name: class_name },
      });
      if (!class_entity) {
        throw new NotFoundException(`Class with name ${class_name} not found`);
      }
      const user_entity = await this.usersRepository.findOne({
      ...
      const existingReview = await this.reviewsRepository.findOne({
        where: { class: { id: class_entity.id }, user: { id: user_entity.id } },
      });
      if (!existingReview) {
        ...
        console.log(
          `Review added for class: ${class_name}, user: ${user_name}`,
        );
      } else {
        console.log(
          `Review already exists for class: ${class_name}, user: ${user_name}`,
        );
      }
  ...
  async getReviewsByClass(classId: string) {
    ...
    if (reviews.length === 0) {
      throw new NotFoundException(
        `No reviews found for class with ID ${classId}`,
      );
    }
    return reviews;
  }
